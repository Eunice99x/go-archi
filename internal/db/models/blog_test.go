...
func testBlogsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(blogPrimaryKeyColumns, blogColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
var (
	blogDBTypes = map[string]string{`ID`: `integer`, `Title`: `character varying`, `Post`: `character varying`}
	_           = bytes.MinRead
)
